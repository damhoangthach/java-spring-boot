...
<script>
    $('#btnAddBuilding').click(function (e) {
        e.preventDefault();
        let data = {}  // mảng này đưa xuống json
        let formData = $('#formEdit').serializeArray();  //lấy data tự động từ form có id editform
        let id = ${modelBuilding.id}+'';
        if ((id) != '') {
            data["id"] = id;
        }
        let buildingTypes = [];
        formData.forEach(item => {    //get data xouoong formdata
            if (item.name == "type") {
        ...
        $.ajax({
            ...
            data: JSON.stringify(data),     //chuyển vè json
